...
function addEditorSettings(
  ...
  function updateEditorSettings(this: HTMLInputElement) {
    ...
    if (optionObj instanceof BoolSelectMultikey) {
      ...
      if (optionObj instanceof NumSelect) {
        ...
        optionObj instanceof BoolSelect ||
        optionObj instanceof BoolRadio
      ) {
        value = value === "true";
      }
  ...
  function createContainer() {
    ...
    for (const option in POSSIBLE_OPTIONS) {
      if (!POSSIBLE_OPTIONS.hasOwnProperty(option)) {
        continue;
      }
      ...
      if (optionObj.label) {
        ...
        if (optionObj instanceof Select) {
          ...
          if (optionObj instanceof NumSelect) {
            ...
            for (let i = 0; i < optionObj.values.length; i++) {
              ...
              if (
                option
                  ...
                  .reduce(
                    (acc, k, j) =>
                      acc && currentOptions[k] === optionObj.values[i][j],
                    true
                  )
              ) {
            ...
            optionObj instanceof BoolSelect ||
            optionObj instanceof TextSelect
          ) {
            for (let i = 0; i < optionObj.values.length; i++) {
              const optionEl = document.createElement("option");
              optionEl.innerHTML = optionObj.valueLabels[i];
        ...
        for (let i = 0; i < optionObj.values.length; i++) {
          ...
          labelEl.setAttribute(
            "for",
            `kae-es${editorSettingsId}-${optionObj.valueLabels[
              i
            ].toLowerCase()}`
          );
          const inputEl = document.createElement("input");
          ...
          inputEl.type = "radio";
          inputEl.id = `kae-es${editorSettingsId}-${optionObj.valueLabels[
            i
          ].toLowerCase()}`;
    ...
    hideEditorToggle.checked =
      localStorage.kaeEditorHidden === "true" ? false : true;
    hideEditorToggle.addEventListener("change", () => {
      const editorWrap = <HTMLDivElement>(
        document.querySelector(".scratchpad-editor-wrap")
      );
      if (editorWrap.parentElement) {
        editorWrap.parentElement.classList.toggle(
          "kae-hidden-editor-wrap",
          !hideEditorToggle.checked
        );
      } else {
      ...
      localStorage.kaeEditorHidden = hideEditorToggle.checked
        ? "false"
        : "true";
    });
